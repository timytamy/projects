...
static int dmxfd = -1;		/* file handle for dmx device */
...
void set()
{
  if(lseek(dmxfd, current_channel*sizeof(dmx_t), SEEK_SET) < 0)
    {
      errorstr="could not seek";
      exit(1);
    }
  if(write(dmxfd, &dmx[current_channel], sizeof(dmx_t)) < 0)
    {
      errorstr="could not write";
      exit(1);
    }
}
...
void setall()
{
  if(lseek(dmxfd, 0, SEEK_SET) < 0)
    {
      errorstr="could not seek";
      exit(1);
    }
  if(write(dmxfd, dmx, MAXCHANNELS*sizeof(dmx_t)) < 0)
    {
      errorstr="could not write";
      exit(1);
    }
...
void getall()
{
  ...
  if(read(dmxfd, dmx, MAXCHANNELS) < 0)
    {
      errorstr="could not read";
      exit(1);
    }
}
...
int main (int argc, const char **argv)
{
  ...
  /* open DMX device */
  dmxfd = open (dev, O_RDWR|O_NONBLOCK);
  if (dmxfd<0)
    {
      fprintf(stderr, "unable to open %s : %s\n", dev, strerror(errno));
      return 1;
  ...
  if(ioctl(dmxfd, DMX_IOCTL_GET_INFO, &info) >= 0)
    {
      int i=info.used_out_universes;
      if(info.used_in_universes > i)
        i=info.used_in_universes;
      i*=512;
      if(i>MAXCHANNELS)
        MAXCHANNELS=i;
    }
